...
import { adminHeaders, adminRoles, adminSession } from "../../app/session";
...
const viewTabs: Array<{
  ...
  { id: "audit", label: "Audit", icon: <LockKeyhole size={18} /> },
  {
    id: "integrations",
    label: "Data Sources",
    icon: <DatabaseZap size={18} />,
  },
  { id: "alertRules", label: "Alert Rules", icon: <BellRing size={18} /> },
];

function AdminConsoleApp() {
  const hasAdminAccess =
    adminRoles.includes(adminSession.role) && adminSession.mfaEnabled;
...
async function fetchAuditLogs(signal?: AbortSignal) {
...
async function fetchDataSources(signal?: AbortSignal) {
...
async function fetchAlertRules(signal?: AbortSignal) {
