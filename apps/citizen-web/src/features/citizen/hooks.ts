 ...
 * reduced-motion by reporting visible immediately. Mirrors the marketing-web
 * primitive so the two public surfaces animate identically (kept local per the
 * no-cross-app-import rule in docs/frontend-structure.md).
 */
 ...
 */
export function useParallax<T extends HTMLElement>(factor = 0.14) {
  const ref = useRef<T>(null);
