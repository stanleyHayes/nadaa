...
import {
  startAgencyMfaEnrollment,
  syncAgencyProfile,
  verifyAgencyMfaEnrollment,
  type AgencySession,
...
export function SecurityTab({
  ...
  const startEnrolment = async () => {
    ...
    setMfaBusy(true);
    setMfaError(null);
    const result = await startAgencyMfaEnrollment(enrollPassword);
    setMfaBusy(false);
  ...
  const verifyMfa = async () => {
    ...
    setMfaBusy(true);
    setMfaError(null);
    const result = await verifyAgencyMfaEnrollment(code, enrollPassword);
    setMfaBusy(false);
    if (!result.ok) {
      setMfaError(result.message);
      return;
      ...
      >
        <Stack spacing={2}>
          ...
          <FieldPanel>
            ...
            {mfaEnabled ? (
              <MutedNote>
                MFA is required for agency accounts. Your agency administrator
                can reset enrollment if you lose your authenticator.
              </MutedNote>
            ) : null}
          </FieldPanel>
