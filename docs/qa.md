...
| Flood ML review              | NADAA-070, NADAA-071, NADAA-072, NADAA-073 | Model, API, E2E    | Confidence, model version, explanation, no auto-publish                                            | Feature pipeline covered; model/API/UI pending |
...
`pnpm smoke:web` expects the citizen app on port `5173`, the authority dashboard compatibility shell on port `5174`, the dispatcher command console on port `5175`, and the admin governance console on port `5176` by default. Override with `LOCAL_CITIZEN_URL`, `LOCAL_AUTHORITY_URL`, `LOCAL_DISPATCHER_URL`, or `LOCAL_ADMIN_URL` when a local port is already occupied.
