...
func requireAuthority(w http.ResponseWriter, r *http.Request) (models.AuthorityContext, bool) {
	...
	if !ok {
		log.Printf("WARN school-service authority_context_missing requestId=%s path=%s", ctx.RequestID, r.URL.Path)
	...
	if !ctx.MFACompleted {
		log.Printf("WARN school-service authority_mfa_required actor=%s role=%s requestId=%s path=%s", ctx.ActorUserID, ctx.ActorRole, ctx.RequestID, r.URL.Path)
	...
	if !authorityRoles[ctx.ActorRole] {
		log.Printf("WARN school-service authority_forbidden actor=%s role=%s requestId=%s path=%s", ctx.ActorUserID, ctx.ActorRole, ctx.RequestID, r.URL.Path)
