...
func TestUSSDMenuAndAlertSummary(t *testing.T) {
	...
	var menuPayload ussdWebhookResponse
	...
	var alertPayload ussdWebhookResponse
	...
	logs := srv.store.listAccessLogs(accessLogFilters{Channel: "ussd", Intent: "current_alerts"})
...
func TestUSSDReportQueuesWhenIncidentServiceIsNotConfigured(t *testing.T) {
	...
	var payload ussdWebhookResponse
	decodeResponse(t, response, &payload)
	...
	logs := srv.store.listAccessLogs(accessLogFilters{Channel: "ussd", Intent: "report_emergency"})
...
func TestSMSInboundReportSubmitsToIncidentService(t *testing.T) {
	var incidentPayload incidentIntakeRequest
	incidentServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, http.StatusCreated, incidentIntakeResponse{ID: "inc_sms_001", Reference: "INC-SMS-001", Status: "reported"})
	}))
	defer incidentServer.Close()

	srv := newTestServer()
	srv.incidentClient = newIncidentServiceClient(incidentServer.URL + "/api/v1")
	...
	var payload smsInboundResponse
	decodeResponse(t, response, &payload)
...
func TestWhatsAppConversationReportQueuesWithLocationAndMedia(t *testing.T) {
	...
	var startPayload whatsappInboundResponse
	...
	var hazardPayload whatsappInboundResponse
	...
	var urgencyPayload whatsappInboundResponse
	...
	var completePayload whatsappInboundResponse
	...
	logs := srv.store.listAccessLogs(accessLogFilters{Channel: "whatsapp", Intent: "report_emergency"})
	hasQueuedLog := false
...
func TestWhatsAppDirectReportSubmitsToIncidentService(t *testing.T) {
	...
	incidentServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, http.StatusCreated, incidentIntakeResponse{ID: "inc_wa_001", Reference: "INC-WA-001", Status: "reported"})
	}))
	defer incidentServer.Close()
	...
	var payload whatsappInboundResponse
	decodeResponse(t, response, &payload)
...
func TestVoiceAlertGenerationReviewAndDelivery(t *testing.T) {
	...
	var created voiceAlertResponse
	...
	var reviewed voiceAlertResponse
	...
	var delivered voiceDeliveryResponse
...
func newTestServer() *server {
	...
	return &server{
		...
		providers: map[string]notificationProvider{
			...
			"voice": mockProvider{channel: "voice"},
		},
