...
func TestPredictionHandlerReturnsFixtureModelPrediction(t *testing.T) {
	srv := newTestServer(t)
	response := postPrediction(t, srv, predictionRequest{
		Location:      coordinates{Lat: 5.5600, Lng: -0.2000},
...
func TestPredictionLogsListStoredPredictions(t *testing.T) {
	srv := newTestServer(t)
	postPrediction(t, srv, predictionRequest{Location: coordinates{Lat: 5.6037, Lng: -0.1870}})
	...
	var payload predictionLogListResponse
...
func newTestServer(t *testing.T) *server {
	t.Helper()
	srv, err := newServerFromModelDir("../../data/flood-risk/models")
	if err != nil {
...
func postPrediction(t *testing.T, srv *server, payload predictionRequest) predictionResponse {
	t.Helper()
